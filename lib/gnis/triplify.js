...
class Triplifier {
		...
		yauzl.open(p_zip_file, {lazyEntries:true}, (e_unzip, k_zip) => {
			...
			k_zip.on('entry', (k_file) => {
				...
				if(k_file.fileName.endsWith('.txt')) {
					...
					// info
					console.log('parsing '+k_file.fileName);
					...
					let ds_writer = ttl_write({
						prefixes: H_PREFIXES,
						coercions: new Map([
							[Date, (dt) => factory.literal(dt.toISOString().replace(/T.+$/, ''), factory.namedNode('http://www.w3.org/2001/XMLSchema#date'))],
						]),
					});

					// pipe output to file
					ds_writer.pipe(fs.createWriteStream(`./data/output/gnis/${s_basename}.ttl`));

					// pass unzip stream to csv parser
					k_zip.openReadStream(k_file, (e_read, ds_csv) => {
						if(e_read) throw e_read;

						this.parse_gnis_csv(ds_csv, ds_writer, k_bar, () => {
							k_zip.readEntry();
						});
					});
				}
	...
	parse_gnis_csv(ds_csv, ds_writer, k_bar, fk_csv) {
		let c_bytes = 0;
		...
		ds_csv.pipe(csv_parser({
			...
			row: (h_row) => {
				...
				// create feature
				let s_feature = h_config.subject(h_row, ds_writer);
				...
				for(let s_property in h_descriptors) {
					...
					if('function' === typeof h_descriptor) {
						let h_add_pairs = h_descriptor.apply(null, [s_value, h_row, ds_writer]);
					...
					// object descriptor
						...
						if('string' === typeof h_descriptor.predicate) {
							...
							if('function' === typeof h_descriptor.object) {
								...
								let w_object = h_descriptor.object.apply(null, [s_value, h_row, ds_writer]);
