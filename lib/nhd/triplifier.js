...
const PD_OUTPUT = path.resolve(__dirname, '../../data/output');
const H_GNIS_FEATURE_LOOKUP = require(`${PD_OUTPUT}/gnis/features.json`);
...
class Triplifier {
	constructor(s_table, i_subset, h_static) {
		...
		// create serializer
		let kt_xsd_date = graphy.namedNode('http://www.w3.org/2001/XMLSchema#date');
		let kt_xsd_integer = graphy.namedNode('http://www.w3.org/2001/XMLSchema#integer');
		let k_serializer = graphy.ttl.serializer({
			prefixes: app_config.prefixes,
			coercions: new Map([
				[Date, (dt) => graphy.literal(dt.toISOString().replace(/T.+$/, ''), kt_xsd_date)],
				[Number, (x) => graphy.literal(x+'', kt_xsd_integer)],
			]),
		});
...
class Triplifier_Feature extends Triplifier {
	...
	async row(h_row) {
		...
		// gnis id
		if(h_row.gnis_id) {
			let sct_gnis_feature = H_GNIS_FEATURE_LOOKUP[+h_row.gnis_id];
			if(sct_gnis_feature) h_pairs['nhd:gnisFeature'] = sct_gnis_feature;
		}
		...
		if(hpg_type.areal) {
			Object.assign(h_geom, {
				...
				// perimeter
				'ago:perimeter': qty_to_wo(h_row.x_perimeter, 'M2'),
