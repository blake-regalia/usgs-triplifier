...
function fcodes(h_fcodes, k_writer) {
	...
	for(let s_fcode in h_fcodes) {
		...
		// subclass
		if(s_kcode.includes(',')) {
			h_pairs['rdfs:subClassOf'] = `nhd:${h_scts[s_kcode.split(',')[0]+'00']}`;
		}
...
const triplify = async (s_basename) => {
	...
	await Promise.all(a_tables
		...
		.map(s_table => (async () => {
			...
			// apply transform
			h_static[s_rename] = f_transform(h_hash, k_writer);
		})()));
	...
	await a_tables
		...
		.map(s_table => (async () => {
			...
			let a_queries = split_query({
				...
				table: s_table,
				query: `
					select *, ${a_select.join(', ')} from ${s_table}`.replace(/\n/g, ' '),
			});
...
if(module.parent) {
	...
	process.argv.slice(2)
		.map(async s => triplify(path.basename(s)))
		.reduce((dp_a, dp_b) => dp_a.then(dp_b), Promise.resolve());
}
